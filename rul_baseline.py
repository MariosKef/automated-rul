...
def preprocessing(cfg, t):
    """
       ...
       """
        ...
        inRUL, reflP = cfg['init_RUL'], cfg['reflection_point']
        train = pd.read_csv('./Data_set'+str(t)+'/train_FD00' + str(t) + '.csv', parse_dates=False, delimiter=" ", decimal=".",
                            header=None)
        ...
        test = pd.read_csv('./Data_set'+str(t)+'/test_FD00' + str(t) + '.csv', parse_dates=False, delimiter=" ", decimal=".",
                            header=None)
        ...
        RUL = pd.read_csv('./Data_set'+str(t)+'/RUL_FD00' + str(t) + '.csv', parse_dates=False, delimiter=" ", decimal=".", header=None)
...
if __name__ == "__main__":
    ...
    for t, rul in zip([1,2,3,4], [115,135,125,135]):
        ...
        print('Pre-processing with reflection point percentage: '+str(cfg['reflection_point'])+'% and init_rul: '+str(cfg['init_RUL']))
        ...
        tpot = TPOTRegressor(generations=GENERATIONS, population_size=POPULATION, verbosity=3, n_jobs=64, scoring=timeliness_score, max_eval_time_mins=MAX_EVAL, random_state=SEED)
        ...
        tpot.export('./'+str(output_file)+'/'+str(date)+'tpot_cmapss_piece_wise_linear_dataset_'+str(t)+'_'+str(GENERATIONS)+'_'+str(POPULATION)+'_'+str(SEED)+'_'+str(MAX_EVAL)+'.py')
